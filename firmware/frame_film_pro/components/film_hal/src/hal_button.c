...
typedef struct
{
    ...
    bool initialized;
    button_callback_t cbs[HAL_BUTTON_PRESS_MAX][BUTTON_MAX_CALLBACKS];
    int cb_counts[HAL_BUTTON_PRESS_MAX];
} button_t;
...
static void trigger_callbacks(hal_button_press_type_t type);
...
static void button_up_cb(void *button_handle, void *usr_data)
{
    ...
    if (event == BUTTON_SINGLE_CLICK)
    {
        ...
        trigger_callbacks(HAL_BUTTON_PRESS_UP);
    }
...
static void button_down_cb(void *button_handle, void *usr_data)
{
    ...
    if (event == BUTTON_SINGLE_CLICK)
    {
        ...
        trigger_callbacks(HAL_BUTTON_PRESS_DOWN);
    }
    else if (event == BUTTON_LONG_PRESS_START)
    {
...
static void button_confirm_cb(void *button_handle, void *usr_data)
{
    ...
    if (event == BUTTON_SINGLE_CLICK)
    {
        ...
        trigger_callbacks(HAL_BUTTON_PRESS_SHORT);
    }
    else if (event == BUTTON_LONG_PRESS_START)
    {
        sys_logi(BUTTON_TAG, "CONFIRM LONG PRESS");
        trigger_callbacks(HAL_BUTTON_PRESS_LONG);
    }
}

static void trigger_callbacks(hal_button_press_type_t type)
{
    if (type < 0 || type >= HAL_BUTTON_PRESS_MAX)
    {
        return;
    }
...
int hal_button_register_cb(hal_button_press_type_t type, button_callback_t cb)
{
    if (!cb) return -1;
    if (type < 0 || type >= HAL_BUTTON_PRESS_MAX) return -4;
...
int hal_button_unregister_cb(hal_button_press_type_t type, button_callback_t cb)
{
    if (!cb) return -1;
    if (type < 0 || type >= HAL_BUTTON_PRESS_MAX) return -4;
    for (int i = 0; i < m_button_mgr.buttons[0].cb_counts[type]; i++)
